...
const Login = () => {
  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    ...
    if (result.error) {
      toast({ title: t("loginError"), description: String(result.error), variant: "destructive" });
    }
    setLoading(false);
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/30 px-4">
      <Card className="w-full max-w-md shadow-card">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-display text-xl">{t("signIn")}</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="ps-10 pe-10" required />
          ...
          <p className="text-center text-sm text-muted-foreground">
            {t("noAccount")}{" "}
            <Link to="/register" className="font-medium text-primary hover:underline">{t("createAccount")}</Link>
          </p>
        </CardContent>
      </Card>
    </div>
